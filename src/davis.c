...
static caerDeviceHandle davisOpenInternal(uint16_t deviceType, uint16_t deviceID, uint8_t busNumberRestrict,
	...
	spiConfigReceive(&state->usbState, DAVIS_CONFIG_DVS, DAVIS_CONFIG_DVS_HAS_POLARITY_FILTER, &param32);
			...
			switch (paramAddr) {
				...
				case DAVIS_CONFIG_APS_EXPOSURE:
					...
					if (!atomic_load(&state->aps.autoExposure.enabled)) {
						...
						return (spiConfigSend(&state->usbState, DAVIS_CONFIG_USB, paramAddr, U32T(exposureCC)));
					}
					else {
						return (false);
					}
				...
				case DAVIS_CONFIG_APS_FRAME_INTERVAL: {
					...
					return (spiConfigSend(&state->usbState, DAVIS_CONFIG_USB, paramAddr, U32T(intervalCC)));
					break;
				}
				...
				case DAVIS_CONFIG_APS_GLOBAL_SHUTTER:
					if (handle->info.apsHasGlobalShutter) {
						...
						if (!spiConfigSend(
								&state->usbState, DAVIS_CONFIG_CHIP, DAVIS128_CONFIG_CHIP_GLOBAL_SHUTTER, param)) {
							return (false);
						}
				...
				switch (paramAddr) {
					...
					case DAVIS128_CONFIG_CHIP_GLOBAL_SHUTTER:
						...
						if (handle->info.apsHasGlobalShutter) {
							...
							if (!spiConfigSend(
									&state->usbState, DAVIS_CONFIG_APS, DAVIS_CONFIG_APS_GLOBAL_SHUTTER, param)) {
								return (false);
							}
...
// Separate debug support.
						...
						case 13: { // APS Column End
							...
							if (state->aps.countY[state->aps.currentReadoutType] != state->aps.roi.sizeY) {
								davisLog(CAER_LOG_ERROR, handle,
									"APS Column End - %d - %d: wrong row count %d detected, expected %d.",
									state->aps.currentReadoutType, state->aps.countX[state->aps.currentReadoutType],
									state->aps.countY[state->aps.currentReadoutType], state->aps.roi.sizeY);
							}
				...
				case 4: {
					...
					if ((state->aps.countX[state->aps.currentReadoutType] >= state->aps.roi.sizeX)
						|| (state->aps.countY[state->aps.currentReadoutType] >= state->aps.roi.sizeY)) {
						break;
					}
					...
					switch (misc8Code) {
						case 0:
							...
							switch (state->imu.count) {
								case 0:
									davisLog(CAER_LOG_ERROR, handle,
										"IMU data: missing IMU Scale Config event. Parsing of IMU events will "
										"still be "
										"attempted, but be aware that Accel/Gyro scale conversions may be "
										"inaccurate.");
									state->imu.count = 1;
				...
				if (atomic_load_explicit(&state->dvs.pixelFilterAutoTrain.autoTrainRunning, memory_order_relaxed)) {
					if (state->dvs.pixelFilterAutoTrain.noiseFilter == NULL) {
						...
						caerFilterDVSNoiseConfigSet(
							state->dvs.pixelFilterAutoTrain.noiseFilter, CAER_FILTER_DVS_HOTPIXEL_COUNT, 10000);
						caerFilterDVSNoiseConfigSet(
							state->dvs.pixelFilterAutoTrain.noiseFilter, CAER_FILTER_DVS_HOTPIXEL_TIME, 2000000);
						caerFilterDVSNoiseConfigSet(
...
bool caerDavisROIConfigure(caerDeviceHandle cdh, uint16_t startX, uint16_t startY, uint16_t endX, uint16_t endY) {
	...
	davisConfigGet(cdh, DAVIS_CONFIG_APS, DAVIS_CONFIG_APS_RUN, &isEnabled);
	...
	for (size_t i = 0; i < commandsNumber; i++) {
		...
		switch (i) {
			case 0: // Disable.
				spiMultiConfig[(SPI_CONFIG_MSG_SIZE * i) + 1] = DAVIS_CONFIG_APS_RUN;
	...
	return (usbControlTransferOut(&handle->state.usbState, VENDOR_REQUEST_FPGA_CONFIG_MULTIPLE, U16T(commandsNumber), 0,
		spiMultiConfig, sizeof(spiMultiConfig)));
}
