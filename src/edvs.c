...
static bool edvsSendBiases(edvsState state);
...
caerDeviceHandle edvsOpen(uint16_t deviceID, uint8_t busNumberRestrict, uint8_t devAddressRestrict,
	...
	// Setup USB.
	usbSetDataCallback(&state->usbState, &edvsEventTranslator, handle);
	...
	edvsLog(CAER_LOG_DEBUG, handle, "Initialized device successfully with USB Bus=%" PRIu8 ":Addr=%" PRIu8 ".",
...
struct caer_edvs_info caerDVS128InfoGet(caerDeviceHandle cdh) {
	edvsHandle handle = (edvsHandle) cdh;
...
bool edvsDataStart(caerDeviceHandle cdh, void (*dataNotifyIncrease)(void *ptr), void (*dataNotifyDecrease)(void *ptr),
	...
	if (atomic_load(&state->dataExchangeStartProducers)) {
		...
		edvsConfigSet((caerDeviceHandle) handle, DVS128_CONFIG_DVS, DVS128_CONFIG_DVS_RUN, true);
	}
...
bool edvsDataStop(caerDeviceHandle cdh) {
	...
	if (atomic_load(&state->dataExchangeStopProducers)) {
		...
		edvsConfigSet((caerDeviceHandle) handle, DVS128_CONFIG_DVS, DVS128_CONFIG_DVS_RUN, false);
	}
...
static void edvsEventTranslator(void *vhd, uint8_t *buffer, size_t bytesSent) {
	...
	if ((bytesSent & 0x03) != 0) {
		edvsLog(CAER_LOG_ALERT, handle, "%zu bytes received via USB, which is not a multiple of four.", bytesSent);
