...
uint32_t dynapseCalculateIndexNeu(uint32_t columns, uint32_t, uint32_t y);
...
static inline void freeAllDataMemory(dynapseState state) {
	...
	if (state->currentSpikePacket != NULL) {
		...
		if (state->currentPacketContainer != NULL) {
			caerEventPacketContainerSetEventPacket(
					state->currentPacketContainer, DYNAPSE_SPIKE_EVENT_POS,
					NULL);
		}
	}
	...
	if (state->currentSpecialPacket != NULL) {
		...
		if (state->currentPacketContainer != NULL) {
			caerEventPacketContainerSetEventPacket(
					state->currentPacketContainer, SPECIAL_EVENT, NULL);
		}
	}
...
caerDeviceHandle dynapseOpen(uint16_t deviceID, uint8_t busNumberRestrict,
	...
	atomic_store_explicit(&state->dataExchangeBufferSize, 64,
			memory_order_relaxed);
	atomic_store_explicit(&state->dataExchangeBlocking, false,
			memory_order_relaxed);
	atomic_store_explicit(&state->dataExchangeStartProducers, true,
			memory_order_relaxed);
	atomic_store_explicit(&state->dataExchangeStopProducers, true,
			memory_order_relaxed);
	...
	atomic_store_explicit(&state->maxPacketContainerPacketSize, 8192,
			memory_order_relaxed);
	...
	if (res != LIBUSB_SUCCESS) {
		free(handle);
		caerLog(CAER_LOG_CRITICAL, __func__,
				"Failed to initialize libusb context. Error: %d.", res);
		return (NULL);
	}
	...
	DYNAPSE_DEVICE_PID,
	DYNAPSE_DEVICE_DID_TYPE, busNumberRestrict, devAddressRestrict,
...
bool dynapseSendDefaultConfig(caerDeviceHandle cdh) {
	dynapseConfigSet(cdh, DYNAPSE_CONFIG_MUX,
	DYNAPSE_CONFIG_MUX_TIMESTAMP_RESET, false);
	dynapseConfigSet(cdh, DYNAPSE_CONFIG_MUX,
	DYNAPSE_CONFIG_MUX_FORCE_CHIP_BIAS_ENABLE, false);
	dynapseConfigSet(cdh, DYNAPSE_CONFIG_MUX,
	DYNAPSE_CONFIG_MUX_DROP_AER_ON_TRANSFER_STALL, false);

	dynapseConfigSet(cdh, DYNAPSE_CONFIG_USB,
	DYNAPSE_CONFIG_USB_EARLY_PACKET_DELAY, 8); // in 125µs time-slices (defaults to 1ms)

	return (true);
}

//i = index, x = amount of columns, y = amount of rows
uint32_t dynapseCalculateCoordinatesNeuX(uint32_t index, uint32_t columns,
		uint32_t rows) {
	//for each row
...
uint32_t dynapseCalculateCoordinatesNeuY(uint32_t index, uint32_t columns,
		uint32_t rows) {
	//for each row
	for (size_t i = 0; i < rows; i++) {
		//check if the index parameter is in the row
...
bool dynapseConfigSet(caerDeviceHandle cdh, int8_t modAddr, uint8_t paramAddr,
		uint32_t param) {
	dynapseHandle handle = (dynapseHandle) cdh;
	dynapseState state = &handle->state;
	...
	case DYNAPSE_CONFIG_MUX:
		...
		case DYNAPSE_CONFIG_MUX_DROP_AER_ON_TRANSFER_STALL:
			...
			DYNAPSE_CONFIG_MUX, paramAddr, param));
			break;
		...
		case DYNAPSE_CONFIG_MUX_TIMESTAMP_RESET: {
			...
			if (param) {
				...
				return (libusb_control_transfer(state->usbState.deviceHandle,
						...
						VENDOR_REQUEST_FPGA_CONFIG_MULTIPLE, 2, 0,
						spiMultiConfig, sizeof(spiMultiConfig), 0)
	...
	case DYNAPSE_CONFIG_AER:
		...
		case DYNAPSE_CONFIG_AER_EXTERNAL_AER_CONTROL:
			return (spiConfigSend(state->usbState.deviceHandle,
			DYNAPSE_CONFIG_AER, paramAddr, param));
			break;
	...
	case DYNAPSE_CONFIG_USB:
		...
		case DYNAPSE_CONFIG_USB_EARLY_PACKET_DELAY:
			return (spiConfigSend(state->usbState.deviceHandle,
			DYNAPSE_CONFIG_USB, paramAddr, param));
			break;
	...
	case DYNAPSE_CONFIG_CLEAR_CAM: {
		...
		while (numConfig > 0) {
			...
			int result = libusb_control_transfer(state->usbState.deviceHandle,
					LIBUSB_ENDPOINT_OUT | LIBUSB_REQUEST_TYPE_VENDOR
							| LIBUSB_RECIPIENT_DEVICE,
					VENDOR_REQUEST_FPGA_CONFIG_AER_MULTIPLE, configNum, 0,
					spiMultiConfig + idxConfig, configSize, 0);
			if (result != configSize) {
				caerLog(CAER_LOG_CRITICAL, handle->info.deviceString,
						"Failed to clear CAM, USB transfer failed with error %d.",
						result);
				return (false);
			}
	...
	case DYNAPSE_CONFIG_MONITOR_NEU: {
		...
		uint32_t coreid = (uint32_t) paramAddr;
		uint32_t bits = 0;
		uint32_t col, row;

		uint8_t spiMultiConfig[6 + 6] = { 0 };  // first reset then set

		uint32_t reset = coreid << 8 | 1 << 11 | 0 << 12 | 0 << 13 | 0 << 16
				| 0 << 17; // reset monitor for this core

		if (neuid >= DYNAPSE_CONFIG_NUMNEURONS_CORE) {
			caerLog(CAER_LOG_CRITICAL, handle->info.deviceString,
					"Failed to monitor neuron, neuron id: %d is invalid.",
					neuid);
			return (false);
		} else {
			...
			caerLog(CAER_LOG_NOTICE, handle->info.deviceString,
					...
					col, row);
			bits = coreid << 8 | 0 << 10 | 0 << 11 | 0 << 12 | 0 << 13 | 0 << 16
					| 0 << 17 | row << 4 | col;
		...
		spiMultiConfig[2] = (reset >> 24) & 0x0FF;
		spiMultiConfig[3] = (reset >> 16) & 0x0FF;
		spiMultiConfig[4] = (reset >> 8) & 0x0FF;
		spiMultiConfig[5] = (reset >> 0) & 0x0FF;

		spiMultiConfig[6] = DYNAPSE_CONFIG_CHIP;
		spiMultiConfig[7] = DYNAPSE_CONFIG_CHIP_CONTENT;
		spiMultiConfig[8] = (bits >> 24) & 0x0FF;
		spiMultiConfig[9] = (bits >> 16) & 0x0FF;
		spiMultiConfig[10] = (bits >> 8) & 0x0FF;
		spiMultiConfig[11] = (bits >> 0) & 0x0FF;

		//usb send
		int result = libusb_control_transfer(state->usbState.deviceHandle,
				LIBUSB_ENDPOINT_OUT | LIBUSB_REQUEST_TYPE_VENDOR
						| LIBUSB_RECIPIENT_DEVICE,
				VENDOR_REQUEST_FPGA_CONFIG_AER_MULTIPLE, 2, 0, spiMultiConfig,
				sizeof(spiMultiConfig), 0);
				...
				for (uint32_t row_neuronid = 0;
					...
					for (uint8_t row_sram = 0;
						...
						spiMultiConfig[(numConfig * 6) + 3] = (bits >> 16)
								& 0x0FF;
						spiMultiConfig[(numConfig * 6) + 4] = (bits >> 8)
								& 0x0FF;
						spiMultiConfig[(numConfig * 6) + 5] = (bits >> 0)
								& 0x0FF;
							...
							for (uint8_t row_sram = 0;
								...
								if (row_sram == 0) {
									...
									bits = row_neuronid << 7 | row_sram << 5
											...
											| distance << 25 | sourcecoreid << 28; // init content chip id and set correct destinations for monitoring
								} else {
									bits = row_neuronid << 7 | row_sram << 5
											| core << 15 | 1 << 17 | 1 << 4; // init content to zero
								}
...
bool dynapseConfigGet(caerDeviceHandle cdh, int8_t modAddr, uint8_t paramAddr,
		uint32_t *param) {
	dynapseHandle handle = (dynapseHandle) cdh;
	dynapseState state = &handle->state;
	...
	case DYNAPSE_CONFIG_MUX:
		...
		case DYNAPSE_CONFIG_MUX_FORCE_CHIP_BIAS_ENABLE:
		case DYNAPSE_CONFIG_MUX_DROP_AER_ON_TRANSFER_STALL:
			return (spiConfigReceive(state->usbState.deviceHandle,
			DYNAPSE_CONFIG_MUX, paramAddr, param));
			break;
	...
	case DYNAPSE_CONFIG_SYSINFO:
		...
		case DYNAPSE_CONFIG_SYSINFO_LOGIC_CLOCK:
			return (spiConfigReceive(state->usbState.deviceHandle,
			DYNAPSE_CONFIG_SYSINFO, paramAddr, param));
			break;
...
bool dynapseDataStart(caerDeviceHandle cdh,
	...
	state->dataExchangeBuffer = ringBufferInit(
			atomic_load(&state->dataExchangeBufferSize));
	if (state->dataExchangeBuffer == NULL) {
		caerLog(CAER_LOG_CRITICAL, handle->info.deviceString,
				"Failed to initialize data exchange buffer.");
		return (false);
	}
	...
	DYNAPSE_SPIKE_DEFAULT_SIZE, I16T(handle->info.deviceID), 0);
	if (state->currentSpikePacket == NULL) {
		freeAllDataMemory(state);

		caerLog(CAER_LOG_CRITICAL, handle->info.deviceString,
				"Failed to allocate spike event packet.");
		return (false);
	}

	state->currentSpecialPacket = caerSpecialEventPacketAllocate(
	DYNAPSE_SPECIAL_DEFAULT_SIZE, I16T(handle->info.deviceID), 0);
	if (state->currentSpecialPacket == NULL) {
		freeAllDataMemory(state);

		caerLog(CAER_LOG_CRITICAL, handle->info.deviceString,
				"Failed to allocate special event packet.");
		return (false);
	}

	if ((errno = thrd_create(&state->dataAcquisitionThread,
			&dynapseDataAcquisitionThread, handle)) != thrd_success) {
		freeAllDataMemory(state);
	...
	while (!atomic_load_explicit(&state->dataAcquisitionThreadRun,
			memory_order_relaxed)) {
		;
	}
...
bool dynapseDataStop(caerDeviceHandle cdh) {
	...
	if (atomic_load(&state->dataExchangeStopProducers)) {
		...
		dynapseConfigSet(cdh, DYNAPSE_CONFIG_AER, DYNAPSE_CONFIG_AER_RUN,
		false);
		dynapseConfigSet(cdh, DYNAPSE_CONFIG_MUX,
		DYNAPSE_CONFIG_MUX_FORCE_CHIP_BIAS_ENABLE, false); // Ensure chip turns off.
		dynapseConfigSet(cdh, DYNAPSE_CONFIG_MUX,
		DYNAPSE_CONFIG_MUX_TIMESTAMP_RUN, false); // Turn off timestamping too.
		dynapseConfigSet(cdh, DYNAPSE_CONFIG_MUX, DYNAPSE_CONFIG_MUX_RUN,
		false);
		dynapseConfigSet(cdh, DYNAPSE_CONFIG_USB, DYNAPSE_CONFIG_USB_RUN,
		false);
	}
	...
	if ((errno = thrd_join(state->dataAcquisitionThread, NULL))
			!= thrd_success) {
		// This should never happen!
		caerLog(CAER_LOG_CRITICAL, handle->info.deviceString,
				"Failed to join data acquisition thread. Error: %d.",
				errno);
		return (false);
	}
...
caerEventPacketContainer dynapseDataGet(caerDeviceHandle cdh) {
	...
	// on blocking setting.
	if (atomic_load_explicit(&state->dataExchangeBlocking,
			memory_order_relaxed)) {
...
static inline int64_t generateFullTimestamp(int32_t tsOverflow,
		int32_t timestamp) {
...
static void dynapseEventTranslator(void *vhd, uint8_t *buffer, size_t bytesSent) {
	...
	if (!atomic_load_explicit(&state->dataAcquisitionThreadRun,
			memory_order_relaxed)) {
		return;
	}
	...
	if ((bytesSent & 0x01) != 0) {
		caerLog(CAER_LOG_ALERT, handle->info.deviceString,
				"%zu bytes received via USB, which is not a multiple of two.",
				bytesSent);
	...
	for (size_t i = 0; i < bytesSent; i += 2) {
		...
		if (state->currentSpikePacket == NULL) {
			state->currentSpikePacket = caerSpikeEventPacketAllocate(
			DYNAPSE_SPIKE_DEFAULT_SIZE, I16T(handle->info.deviceID),
					state->wrapOverflow);
			if (state->currentSpikePacket == NULL) {
				caerLog(CAER_LOG_CRITICAL, handle->info.deviceString,
						"Failed to allocate spike event packet.");
			...
			caerSpikeEventPacket grownPacket =
					(caerSpikeEventPacket) caerGenericEventPacketGrow(
							(caerEventPacketHeader) state->currentSpikePacket,
							state->currentSpikePacketPosition * 2);
			if (grownPacket == NULL) {
				caerLog(CAER_LOG_CRITICAL, handle->info.deviceString,
						"Failed to grow spike event packet.");
				return;
			}
		...
		if (state->currentSpecialPacket == NULL) {
			state->currentSpecialPacket = caerSpecialEventPacketAllocate(
			DYNAPSE_SPECIAL_DEFAULT_SIZE, I16T(handle->info.deviceID),
					state->wrapOverflow);
			if (state->currentSpecialPacket == NULL) {
				caerLog(CAER_LOG_CRITICAL, handle->info.deviceString,
						"Failed to allocate special event packet.");
			...
			caerSpecialEventPacket grownPacket =
					(caerSpecialEventPacket) caerGenericEventPacketGrow(
							(caerEventPacketHeader) state->currentSpecialPacket,
							state->currentSpecialPacketPosition * 2);
			if (grownPacket == NULL) {
				caerLog(CAER_LOG_CRITICAL, handle->info.deviceString,
						"Failed to grow special event packet.");
				return;
			}
		...
		if ((event & 0x8000) != 0) {
			...
			case 0: // Special event
				...
				case 1: { // Timetamp reset
					...
					atomic_fetch_or(&state->dataAcquisitionThreadConfigUpdate,
							1 << 1);
				...
				default:
					caerLog(CAER_LOG_ERROR, handle->info.deviceString,
							"Caught special event that can't be handled: %d.",
							data);
					break;
				}
				break;
			...
			case 6: {
				...
				caerSpikeEventValidate(currentSpikeEvent,
						state->currentSpikePacket);
				state->currentSpikePacketPosition++;
				...
				if (wrapSum > I64T(INT32_MAX)) {
					...
					caerSpecialEvent currentSpecialEvent =
							caerSpecialEventPacketGetEvent(
									state->currentSpecialPacket,
									state->currentSpecialPacketPosition);
					caerSpecialEventSetTimestamp(currentSpecialEvent,
					INT32_MAX);
					caerSpecialEventSetType(currentSpecialEvent,
							TIMESTAMP_WRAP);
					caerSpecialEventValidate(currentSpecialEvent,
							state->currentSpecialPacket);
					state->currentSpecialPacketPosition++;
					...
					caerLog(CAER_LOG_DEBUG, handle->info.deviceString,
							"Timestamp wrap event received with multiplier of %" PRIu16 ".",
							data);
				}
			...
			default:
				caerLog(CAER_LOG_ERROR, handle->info.deviceString,
						"Caught event that can't be handled.");
				break;
			}
		}
		...
		int32_t currentPacketContainerCommitSize =
				I32T(
		...
		bool containerSizeCommit = (currentPacketContainerCommitSize > 0)
				&& ((state->currentSpikePacketPosition
						>= currentPacketContainerCommitSize)
						|| (state->currentSpecialPacketPosition
								>= currentPacketContainerCommitSize));

		bool containerTimeCommit = generateFullTimestamp(state->wrapOverflow,
				state->currentTimestamp)
				> state->currentPacketContainerCommitTimestamp;
		...
		if (tsReset || tsBigWrap || containerSizeCommit
				|| containerTimeCommit) {
			...
			if (state->currentSpecialPacketPosition > 0) {
				caerEventPacketContainerSetEventPacket(
						state->currentPacketContainer, SPECIAL_EVENT,
						(caerEventPacketHeader) state->currentSpecialPacket);
			...
			if (containerTimeCommit) {
				while (generateFullTimestamp(state->wrapOverflow,
						state->currentTimestamp)
						> state->currentPacketContainerCommitTimestamp) {
					state->currentPacketContainerCommitTimestamp +=
							I32T(
			...
			if (emptyContainerCommit) {
				...
				if (!ringBufferPut(state->dataExchangeBuffer,
						state->currentPacketContainer)) {
			...
			if (tsReset) {
				...
				caerEventPacketContainer tsResetContainer =
						caerEventPacketContainerAllocate(DYNAPSE_EVENT_TYPES);
				...
				caerSpecialEventPacket tsResetPacket =
						caerSpecialEventPacketAllocate(1,
								I16T(handle->info.deviceID),
								state->wrapOverflow);
				...
				caerSpecialEvent tsResetEvent = caerSpecialEventPacketGetEvent(
						tsResetPacket, 0);
				...
				caerEventPacketContainerSetEventPacket(tsResetContainer,
						SPECIAL_EVENT, (caerEventPacketHeader) tsResetPacket);
				...
				while (!ringBufferPut(state->dataExchangeBuffer,
						tsResetContainer)) {
...
static int dynapseDataAcquisitionThread(void *inPtr) {
	...
	caerLog(CAER_LOG_DEBUG, handle->info.deviceString,
			"Initializing data acquisition thread ...");
	...
	if (atomic_load(&state->dataExchangeStartProducers)) {
		...
		dynapseConfigSet((caerDeviceHandle) handle, DYNAPSE_CONFIG_USB,
		DYNAPSE_CONFIG_USB_RUN, true);
		dynapseConfigSet((caerDeviceHandle) handle, DYNAPSE_CONFIG_MUX,
		DYNAPSE_CONFIG_MUX_RUN, true);
		dynapseConfigSet((caerDeviceHandle) handle, DYNAPSE_CONFIG_MUX,
		DYNAPSE_CONFIG_MUX_TIMESTAMP_RUN, true);
		dynapseConfigSet((caerDeviceHandle) handle, DYNAPSE_CONFIG_AER,
		DYNAPSE_CONFIG_AER_RUN, true);
	}

	// Create buffers as specified in config file.
	usbAllocateTransfers(&state->usbState,
			U32T(atomic_load(&state->usbBufferNumber)),
	...
	caerLog(CAER_LOG_DEBUG, handle->info.deviceString,
			"data acquisition thread ready to process events.");

	// Handle USB events (1 second timeout).
	struct timeval te = { .tv_sec = 1, .tv_usec = 0 };

	while (atomic_load_explicit(&state->dataAcquisitionThreadRun,
			memory_order_relaxed) && state->usbState.activeDataTransfers > 0) {
		// Check config refresh, in this case to adjust buffer sizes.
		if (atomic_load_explicit(&state->dataAcquisitionThreadConfigUpdate,
	...
	caerLog(CAER_LOG_DEBUG, handle->info.deviceString,
			"shutting down data acquisition thread ...");
	...
	caerLog(CAER_LOG_DEBUG, handle->info.deviceString,
			"data acquisition thread shut down.");
...
static void dynapseDataAcquisitionThreadConfig(dynapseHandle handle) {
	...
	uint32_t configUpdate = U32T(
			atomic_exchange(&state->dataAcquisitionThreadConfigUpdate, 0));
